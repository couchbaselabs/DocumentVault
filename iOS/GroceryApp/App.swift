...
final class GrocerySyncApp {
    ...
    var endpoint: GrocerySyncApp.Endpoint? {
        didSet {
    ...
    private let networkQueue = DispatchQueue(label: "GrocerySyncNetworkQueue", target: .global())
    ...
    init(database: CouchbaseLiteSwift.Database, endpoint: GrocerySyncApp.Endpoint? = nil, identity: SecIdentity? = nil, ca: SecCertificate? = nil) {
        ...
        print("[GrocerySync] P2P sync configured")
    ...
    private func createListener() -> NWListener? {
        ...
        listener.stateUpdateHandler = { [weak self] newState in
            switch newState {
            case .ready:
                Log.info("P2P Listener ready for grocery inventory sync")
        ...
        browser.browseResultsChangedHandler = { [weak self] results, changes in
            for change in changes {
                ...
                case .removed(let result):
                    Log.info("Lost grocery sync peer: \(result.endpoint)")
    ...
    private func handleIncomingConnection(_ connection: NWConnection) {
        Log.info("New incoming grocery sync connection: \(connection)")
    ...
    private func handleDiscoveredPeer(_ endpoint: NWEndpoint) {
        ...
        Log.info("Connecting to discovered grocery sync peer: \(connection)")
    ...
    private func setupConnection(_ connection: NWConnection, isInitiator: Bool) {
        connection.stateUpdateHandler = { [weak self] newState in
            switch newState {
            case .ready:
                Log.info("Grocery sync connection ready: \(connection)")
            ...
            case .failed(let error):
                Log.error("Grocery sync connection failed: \(error)")
                self?.cleanupConnection(connection)
            case .cancelled:
                Log.info("Grocery sync connection cancelled: \(connection)")
                self?.cleanupConnection(connection)
            default:
                break
            }
    ...
    private class NMMessageEndpointConnection: CouchbaseLiteSwift.MessageEndpointConnection {
        ...
        func send(message: CouchbaseLiteSwift.Message, completion: @escaping (Bool, CouchbaseLiteSwift.MessagingError?) -> Void) {
            ...
            Log.info("Sending grocery data: \(data.count) bytes")
        ...
        private func receive() {
            connection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { [weak self] (data, _, _, error) in
                if let error = error {
                    ...
                    if let data = data {
                        Log.info("Received grocery data: \(data.count) bytes")
    ...
    private func requestNetworkPermissionAlternative() async {
        ...
        let queue = DispatchQueue(label: "com.couchbase.groceryapp.networkAuthAlt")
    ...
    private func requestLocalNetworkAuthorization() async throws -> Bool {
        let queue = DispatchQueue(label: "com.couchbase.groceryapp.networkAuth")
        let serviceType = "_groceryapp._tcp" // Must match NSBonjourServices in Info.plist
    ...
    class Log {
        static private let logger = OSLog(subsystem: "grocery-inventory", category: "sync")
