...
// GroceryCRDTConflictResolver resolves conflicts for documents that contain
...
class GroceryCRDTConflictResolver: ConflictResolverProtocol {
    static let shared: GroceryCRDTConflictResolver = GroceryCRDTConflictResolver()
        ...
        for key in localAndRemoteKeys {
            ...
            if localDocument[key].dictionary?["type"].string == "pn-counter" || 
               remoteDocument[key].dictionary?["type"].string == "pn-counter" {
                ...
                print("[GrocerySync] Resolving CRDT conflict for key: \(key)")
                ...
                print("[GrocerySync] CRDT conflict resolved: \(key) = \(finalValue)")
            }
        }
...
extension CouchbaseLiteSwift.Database {
    var deviceUUID: String {
        ...
        guard let uuid = UserDefaults.standard.string(forKey: key) else {
            ...
            print("[GrocerySync] Generated new device UUID: \(uuid)")
            return uuid
        }
        return uuid
    }
