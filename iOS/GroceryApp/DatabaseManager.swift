...
extension Notification.Name {
    static let groceryInventoryChanged = Notification.Name("groceryInventoryChanged")
}
...
class DatabaseManager: ObservableObject {
    ...
    deinit {
        ...
        print("🧹 [GrocerySync] DatabaseManager cleaned up change listeners")
    }
    
    private func setupChangeListeners() {
        guard let database = database else { 
            print("❌ [GrocerySync] No database available for change listener setup")
            return 
        }
        
        do {
            print("🔧 [GrocerySync] Setting up change listeners...")
            print("🔧 [GrocerySync] Using scope: \(AppConfig.scopeName), collection: \(collectionName)")
            ...
            collectionChangeListenerToken = collection.addChangeListener { change in
                print("🔔🔔🔔 [GrocerySync] COLLECTION CHANGE LISTENER TRIGGERED!")
                print("🔧 [GrocerySync] Changed document IDs: \(change.documentIDs)")
                print("🔧 [GrocerySync] Document count: \(change.documentIDs.count)")
                
                // Always execute on main queue for UI updates (like the working sample)
                DispatchQueue.main.async { [weak self] in
                    guard let self = self else { return }
                    
                    print("🔄🔄🔄 [GrocerySync] Collection changed: \(change.documentIDs.count) documents")
                    for docId in change.documentIDs {
                        print("📄 [GrocerySync] Collection document changed: \(docId)")
                    }
                    ...
                    NotificationCenter.default.post(name: .groceryInventoryChanged, object: nil)
                    print("📡📡📡 [GrocerySync] Posted collection inventory changed notification")
                }
            }
            
            print("✅ [GrocerySync] Collection change listener configured for '\(collectionName)'")
            print("🔧 [GrocerySync] Collection token: \(collectionChangeListenerToken != nil ? "✅" : "❌")")
        } catch {
    ...
    /// ✨ REACTIVE API: Save a GroceryItem using collection.save(from:)
    ...
    func saveGroceryItem(_ item: GroceryItem) {
        ...
        do {
            ...
            // CodingKeys in GroceryItem automatically map fields:
            ...
            print("✅ [Reactive API] Saved grocery item: \(item.name) (auto-encoded via Codable)")
        } catch {
            print("❌ [Reactive API] Error saving grocery item \(item.name): \(error)")
        }
    }
        ...
        do {
            ...
            for result in results {
                ...
                if let dict = result.dictionary(forKey: collectionName) {
                    ...
                    // Read location
                    var location: GroceryItem.Location? = nil
                    ...
                    // Read attributes
                    var attributes: GroceryItem.Attributes? = nil
                    ...
                    //print("Retrieved grocery item: \(name) (qty: \(quantity))")
                } else {
                    //print("Failed to parse result \(resultCount)")
                }
            }
            
            print("Retrieved \(groceryItems.count) grocery items from database")
            return groceryItems
        } catch {
    ...
    func updateQuantity(for itemId: String, newQuantity: Int) {
        print("🔧 [GrocerySync] updateQuantity called for \(itemId): new=\(newQuantity)")
        guard let database = database else { 
            print("❌ [GrocerySync] Database not available")
            return 
        }
        
        do {
            guard let collection = try database.collection(name: collectionName, scope: AppConfig.scopeName) else { 
                print("❌ [GrocerySync] Collection \(collectionName) in scope \(AppConfig.scopeName) not available")
                return 
            }
            
            // Get current document and quantity
            guard let document = try collection.document(id: itemId) else {
                print("❌ [GrocerySync] Document not found for item \(itemId)")
                return
            }
            ...
            print("🔧 [GrocerySync] Current: \(currentQuantity), New: \(newQuantity), Diff: \(difference)")
            
            if difference > 0 {
                print("🔧 [GrocerySync] Incrementing by \(difference)")
                incrementQuantity(for: itemId, by: UInt(difference), in: collection)
            } else if difference < 0 {
                print("🔧 [GrocerySync] Decrementing by \(-difference)")
                decrementQuantity(for: itemId, by: UInt(-difference), in: collection)
            } else {
                print("🔧 [GrocerySync] No change needed (difference = 0)")
            }
            
        } catch {
            print("❌ [GrocerySync] Error updating quantity: \(error)")
        }
    }
    ...
    private func incrementQuantity(for itemId: String, by amount: UInt, in collection: Collection) {
        ...
        while !saved && attempts < maxAttempts {
            ...
            do {
                ...
                if saved {
                    ...
                    print("✅ [GrocerySync] Incremented quantity for \(itemId)")
                    ...
                    DispatchQueue.main.async {
                        NotificationCenter.default.post(name: .groceryInventoryChanged, object: nil)
                        print("📡 [GrocerySync] Posted inventory changed notification for LOCAL change")
                    }
                } else {
                    print("[GrocerySync] Increment failed, retrying... (attempt \(attempts))")
                }
            } catch {
        ...
        if !saved {
            print("[GrocerySync] Failed to increment quantity after \(maxAttempts) attempts")
        }
    }
    ...
    private func decrementQuantity(for itemId: String, by amount: UInt, in collection: Collection) {
        ...
        while !saved && attempts < maxAttempts {
            ...
            do {
                ...
                if saved {
                    let newValue = getCurrentQuantity(from: document)
                    print("✅ [GrocerySync] Decremented quantity for \(itemId)")
                    print("   📊 Change: \(oldValue) → \(newValue) (-\(amount))")
                    print("   🎭 Actor: \(database?.deviceUUID ?? "unknown")")
                    print("   ⏱️  This change should sync to other devices NOW!")
                    ...
                    print("[GrocerySync] Decrement failed, retrying... (attempt \(attempts))")
                }
            } catch {
        ...
        if !saved {
            print("[GrocerySync] Failed to decrement quantity after \(maxAttempts) attempts")
        }
    }
    ...
    func searchGrocery(_ searchText: String) -> [GroceryItem] {
        guard let database = database else { return [] }
        ...
        do {
            ...
            print("Search for '\(searchText)' returned \(groceryItems.count) items")
            return groceryItems
        } catch {
            print("Error searching grocery: \(error)")
            return []
        }
    }
    ...
    func createOrder(item: GroceryItem, quantity: Int = 100) -> Order? {
        guard let database = database else { return nil }
    ...
    func createGroceryItemWithSync(name: String, type: String, price: Double, imageURL: String, quantity: Int = 0) -> String? {
        // Create via App Services if enabled (will also save locally)
        if isAppServicesEnabled, let itemId = appServicesSyncManager?.createGroceryItem(
            name: name, 
            type: type, 
            price: price, 
            imageURL: imageURL, 
        ...
        let item = GroceryItem(name: name, type: type, price: price, imageURL: imageURL, quantity: quantity)
