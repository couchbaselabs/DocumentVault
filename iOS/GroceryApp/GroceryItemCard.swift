...
struct GroceryItemCard: View {
    ...
    let onReorder: (GroceryItem, Int) -> Void
    let storeId: String
    @State private var currentQuantity: Int
    @State private var showOrderPlaced = false
    @State private var showOrderForm = false
    
    init(item: GroceryItem, storeId: String = "", onQuantityChanged: @escaping (Int) -> Void, onReorder: @escaping (GroceryItem, Int) -> Void = { _, _ in }) {
        self.item = item
...
#Preview {
    HStack {
        ...
        GroceryItemCard(
            item: GroceryItem(
                name: "Very Long Grocery Name That Should Be Truncated",
                type: "Produce",
                price: 12.99,
                imageURL: "produce1",
                quantity: 2
