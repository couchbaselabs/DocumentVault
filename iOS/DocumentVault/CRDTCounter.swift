...
// VaultCRDTConflictResolver resolves conflicts for documents that contain
...
class VaultCRDTConflictResolver: ConflictResolverProtocol {
    static let shared: VaultCRDTConflictResolver = VaultCRDTConflictResolver()
        ...
        for key in localAndRemoteKeys {
            ...
            if localDocument[key].dictionary?["type"].string == "pn-counter" || 
               remoteDocument[key].dictionary?["type"].string == "pn-counter" {
                ...
                print("[VaultSync] Resolving CRDT conflict for key: \(key)")
                ...
                print("[VaultSync] CRDT conflict resolved: \(key) = \(finalValue)")
            }
        }
...
extension CouchbaseLiteSwift.Database {
    var deviceUUID: String {
        ...
        guard let uuid = UserDefaults.standard.string(forKey: key) else {
            ...
            print("[VaultSync] Generated new device UUID: \(uuid)")
            return uuid
        }
        return uuid
    }
